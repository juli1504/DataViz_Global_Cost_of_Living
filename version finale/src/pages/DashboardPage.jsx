...
const DashboardPage = ({ data, countries }) => {
  ...
  const yearRange = useMemo(() => {
    if (!data || data.length === 0) return { min: 1970, max: 2021 };

    let relevantData = [];

    if (activeTab === 'pulse') {
      relevantData = data.filter(d => d.country === pulseSelection);
    } else {
      if (compareSelection.length > 0) {
        relevantData = data.filter(d => compareSelection.includes(d.country));
      } else {
        relevantData = data;
      }
    }

    if (relevantData.length === 0) return { min: 1970, max: 2021 };

    return {
      min: Math.min(...relevantData.map(d => d.year)),
      max: Math.max(...relevantData.map(d => d.year))
    };
  }, [data, activeTab, pulseSelection, compareSelection]);

  // Si on est en 2021 et qu'on sélectionne l'URSS (max 1990), on force le retour à 1990
  useEffect(() => {
    if (currentYear > yearRange.max) {
      setCurrentYear(yearRange.max);
    } else if (currentYear < yearRange.min) {
      setCurrentYear(yearRange.min);
    }
  }, [yearRange, currentYear, setCurrentYear]);

  const currentSelectionList = activeTab === 'pulse' ? [pulseSelection] : compareSelection;
  
  const pulseData = useMemo(() => {
    return data.filter(d => d.country === pulseSelection);
  }, [data, pulseSelection]);

  const compareDatasets = useMemo(() => {
    return compareSelection.map(country => ({
      country,
      data: data.filter(d => d.country === country)
    }));
  }, [data, compareSelection]);
      ...
      <div className="w-80 flex-none bg-slate-900/50 border-r border-white/5 backdrop-blur-md flex flex-col p-6 z-20 h-full">
        <div className="flex-none">
          <div className="flex flex-col gap-2 mb-4">
            <NavButton active={activeTab === 'pulse'} onClick={() => setActiveTab('pulse')} icon="🧿" label="Résumé Pays" desc="Focus Année & Structure" />
            <NavButton active={activeTab === 'compare'} onClick={() => setActiveTab('compare')} icon="📊" label="Comparaisons" desc="Historique & Secteurs" />
          </div>
        ...
        <div className="h-40 flex-none flex flex-col border-b border-white/5 pb-4 mb-4">
            <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 flex justify-between items-center">
              <span>{activeTab === 'pulse' ? 'PAYS CIBLE' : 'SÉLECTION COMPARAISON'}</span>
              <span className="bg-slate-800 px-2 py-0.5 rounded text-cyan-500">{currentSelectionList.length}</span>
            </div>
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 space-y-2">
               {currentSelectionList.map((country, idx) => (
                  <div key={country} className="flex items-center justify-between bg-slate-800/80 border border-slate-600 rounded-lg pl-3 pr-2 py-2 group hover:border-red-500/50 transition-colors">
                     ...
                     {(activeTab === 'compare' && currentSelectionList.length > 1) && (
        ...
        <div className="flex-1 min-h-0 flex flex-col">
          <span className="text-[10px] text-slate-500 uppercase mb-2">
            {activeTab === 'pulse' ? 'Changer de pays' : 'Ajouter un pays'}
          </span>
          <div className="flex-1 overflow-hidden">
             <CountrySelector countries={countries} selectedCountries={currentSelectionList} onToggle={toggleCountry} />
          </div>
        </div>
      </div>
          ...
          {activeTab === 'pulse' && (
            <div className="flex-1 flex flex-col items-center justify-center pt-12">
              <div className="text-center mb-2 animate-enter relative z-0 flex-none">
                <h2 className="text-5xl font-bold text-white mb-1 tracking-tight">{pulseSelection}</h2>
              </div>

              <div className="flex-1 w-full relative">
                 <EconomicPulse data={pulseData} year={currentYear} country={pulseSelection} />
              </div>
            </div>
          )}
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                            <div className="w-full max-w-3xl h-full flex flex-col bg-slate-800/40 border border-white/5 rounded-2xl p-4">
                                <div className="flex items-center gap-2 mb-2 justify-center flex-none">
                                    ...
                                    <h3 className="font-bold text-white text-lg">{currentSelectionList[0]}</h3>
                                </div>
